...
int main(int argc, char* args[])
{
	...
	if (game->check_integrity())
	{
		...
		Grid* grid = new Grid(game->get_renderer(), 0, 0, 100, 100, 20, 20);

		Game_Entity* window_entity = new Game_Entity(game->get_renderer());
		Vector2 window_dimension = game->get_win_dimensions();
		window_entity->add_grid(grid);
		...
		const std::vector<const Game_Entity*> entities = { yellow_apple, green_apple, red_apple, window_entity };
